...
public class Dial4JaCaArtifact extends Artifact implements IAgent {
	...
	void replyWithContext(String response, OutputContexts context) {
		System.out.println("[Rasa4JaCa] Reply received from agent");
		this.jasonResponse = response;
	...
	void reply(String response) {
		if (this.awaitingResponse) {
			...
			System.out.println("[Rasa4JaCa] Reply arrived late");
		}
	}
	...
	public ResponseRasa RasaIntentionProcessing(String intentName, HashMap<String, Object> parameters, String session) {
		...
		this.jasonOutputParameters = (HashMap<String,Object>)parameters;
		this.awaitingResponse = true;

		ResponseRasa response = new ResponseRasa();
		if (!this.generatedEvent || !this.intentEvent.equals(intentName)) {
			this.generatedEvent = false;
			this.intentEvent = "";
			if (intentName != null) {
				execInternalOp("createRequestBelief", session, intentName, parameters, null, session);
				System.out.println("[Rasa4JaCa] Defining observable property");
			} else {
				System.out.println("[Rasa4JaCa] Could not set observable property");
				response.setResponse("Unrecognized intent");
			}
		}
		...
		if (this.jasonResponse != null) {
			System.out.println("[Rasa4JaCa] Agent jason's response: " + this.jasonResponse);
			response.setResponse(this.jasonResponse);
			this.jasonResponse = null;
			this.awaitingResponse = false;
			this.generatedEvent = false;
			this.intentEvent = "";
		} else {
			System.out.println("[Rasa4JaCa] No response from agent jason");
		}
